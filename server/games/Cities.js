      ...
      try {
        process.send({
          ...
          data: {
            m: 'godmsg',
            s: sniffer.sid,
            msg: '[' + Lib.humanTimeDate(Date.now()) + '] [' + WORKER_INDEX + '-' + WORKER_NAME + '] [game_cities] ' + ' Sniffing Activated!'
          }
        });
      } catch(err) {
        console.log(err);
        ...
        try {
          process.send({
            ...
            data: {
              m: 'godmsg',
              s: sniffer.sid,
              msg: '[' + Lib.humanTimeDate(Date.now()) + '] [' + WORKER_INDEX + '-' + WORKER_NAME + '] [game_cities] ' + ' Sniffing De-Activated!'
            }
          });
        } catch(err) {
          console.log(err);
      ...
      try {
        process.send({
          ...
          data: {
            m: 'godmsg',
            s: snooper.sid,
            msg: '[' + Lib.humanTimeDate(Date.now()) + '] [' + WORKER_INDEX + '-' + WORKER_NAME + '] [game_cities] ' + ' Snooping Activated!'
          }
        });
      } catch(err) {
        console.log(err);
        ...
        try {
          process.send({
            ...
            data: {
              m: 'godmsg',
              s: snooper.sid,
              msg: '[' + Lib.humanTimeDate(Date.now()) + '] [' + WORKER_INDEX + '-' + WORKER_NAME + '] [game_cities] ' + ' Snooping De-Activated!'
            }
          });
        } catch(err) {
          console.log(err);
...
class Game {
  static setup(){
    ...
    this.gameid = 'g' + Lib.md5(Math.random() + Date.now()) + 'game_cities';
    log('Setup game_cities ' + this.gameid);
    this.pointpool = 0;
    this.allowplayers = {};
    this.players = [];
  ...
  static start(){
    ...
    log('Starting game_cities with ' + this.playersalive + ' players ' + this.gameid);
  ...
  static endgame(){
    ...
    // keep track of players
    log('Ending game_cities. Time: ' + Lib.humanTimeDiff(this.starttime, Date.now()) + ' Alive: ' + this.playersalive + ' Game: ' + this.gameid);
    ...
    try {
      process.send({
        ...
        data: {
          m: 'godmsg',
          s: e.sid,
          msg: '[' + Lib.humanTimeDate(Date.now()) + '] [' + WORKER_INDEX + '-' + WORKER_NAME + '] [game_cities] ' + logmsg
        }
      });
    } catch(err) {
      console.log(err);
    ...
    try {
      process.send({
        ...
        data: {
          m: 'godmsg',
          s: e.sid,
          msg: '[' + Lib.humanTimeDate(Date.now()) + '] [' + WORKER_INDEX + '-' + WORKER_NAME + '] [game_cities] ' + msg
        }
      });
    } catch(err) {
      console.log(err);
...
module.exports.setup = function (p) {
  ...
  log('Hi I\'m worker ' + WORKER_INDEX + ' running as a game_cities room. {' + WORKER_NAME + '}{' + NODE_ENV + '}');
      ...
      try {
        process.send({
          ...
          data: {
            m: 'godmsg',
            s: m.sid,
            msg: '[' + WORKER_INDEX + '-' + WORKER_NAME + '] [game_cities]' + ' Uptime:' + Lib.humanTimeDiff(uptime, Date.now()) + ' Clients:' + wss.clients.length +
        ...
        Game.chatlogs.forEach((e,i)=>{
          process.send({
            ...
            data: {
              m: 'godmsg',
              s: m.sid,
              msg: '[' + WORKER_INDEX + '-' + WORKER_NAME + '] [game_cities] ' + e
            }
          });
        });
      } catch(err) {
